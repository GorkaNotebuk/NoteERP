 ...
 */
class Tiposdedocumentos extends CommonObject
{
	public $element='tiposdedocumentos';
	public $table_element='tiposdedocumentos';
	protected $ismultientitymanaged = 1;	// 0=No test on entity, 1=Test with field entity, 2=Test with link by societe


var $denominacion;			// Name of group
	var $validezdias;			// Name of group
...
*/
	...
	function fetch($id='', $groupname='')
	{
		...
		$sql = "SELECT docID,denominacion,validezdias";
		$sql.= " FROM ".MAIN_DB_PREFIX."tiposdedocumentos";
		/*if ($groupname)
		{
		...
		if ($result)
		{
			if ($this->db->num_rows($result))
			{
				...
				$this->validezdias = $obj->validezdias;
	...
	/**
	 *        borrar el tipo de documento de la base de datos
	 *
	...
	function delete()
	{
		...
		$sql = "DELETE FROM ".MAIN_DB_PREFIX."tiposdedocumentos";
		$sql .= " WHERE docID = ".$this->id;
		$result=$this->db->query($sql);
		$this->db->commit();
		/*if ($result)
		{
	...
	/**
	 *	Crear el nuevo tipo de documento en la base de datos ////Corresponde a la accion ($action == 'add') de card.php de la carpeta documentos
	 *
	...
	function create($notrigger=0)
	{
		...
		$error=0;
		$now=dol_now(); ///La fecha del día de hoy
...
*/
		...
		$sql = "INSERT INTO ".MAIN_DB_PREFIX."tiposdedocumentos (denominacion, validezdias) VALUES ('".$this->db->escape($this->denominacion)."',".$this->db->escape($this->validezdias).")";
		
		/*$sql = "INSERT INTO ".MAIN_DB_PREFIX."tiposdedocumentos (";
		$sql.= "denominacion";
		
		$sql.= ", validezdias";
		$sql.= ") VALUES (";
		//$sql.= "'".$this->db->idate($now)."'";
		//$sql.= ",'".$this->db->escape($this->denominacion)."'";
		//$sql.= ",".$this->db->escape($this->validezdias) ;
		$sql.="'".$h."'";
		$sql.="'".$nid."'";
		...
		/*if ($result)
		{
			$this->id = $this->db->last_insert_id(MAIN_DB_PREFIX."tiposdedocumentos");
...
*/
		...
		$sql = "UPDATE ".MAIN_DB_PREFIX."tiposdedocumentos SET ";
		$sql.= " denominacion = '" . $this->db->escape($this->denominacion) . "'";
		//$sql.= ", entity = " . $this->db->escape($entity);
		$sql.= ", validezdias = '" . $this->db->escape($this->validezdias) . "'";
		$sql.= " WHERE docID = " . $this->id;
