...
require_once DOL_DOCUMENT_ROOT.'/user/class/documento.class.php';
...
//Esto lo he comentado porque no me deja entrar a editar los tipos de documentos
...
////Creamos un nuevo objeto de tipo Tiposdedocumentos, cuya clase está creadad en el documento.class.php y todo lo de dentro son metodos de la clase
///En esta clase hemos sacado los valores de la base de datos.
$object = new Tiposdedocumentos($db);
...
/**
 *  Accion de añadir un nuevo tipo de documento a la BD que es llamado desde action=create
 */
if ($action == 'add')
{
    if ($caneditperms)
    {
        if (! $_POST["denominacion"]) {
            setEventMessage("Faltan campos por rellenar", 'errors');
            $action="create";       // Go back to create page
        } else if (! $_POST["validezdias"]) {
            setEventMessage("Faltan campos por rellenar", 'errors');
            $action="create";       // Go back to create page
        } 
        else {
            //$object->denominacion = trim($_POST["denominacion"]); // For backward compatibility
            $object->denominacion   = trim($_POST["denominacion"]);
            $object->validezdias    = trim($_POST["validezdias"]);
            ...
            $object->create();
             ...
             header("Location: ./index.php"); ////Redirige a la lista de los tipos de documentos
             exit;

/*
            $id = $object->create(); //De aqui vamos a la ../class/clase documento.class.php a la funcion create
...
if ($action == 'update')
{
    if ($caneditperms)
    {
        ...
        $object->denominacion   = trim($_POST["denominacion"]);
       // $object->denominacion    = $object->denominacion;            // For backward compatibility
        $object->validezdias   = dol_htmlcleanlastbr($_POST["validezdias"]);
...
if ($action == 'create')
{
    print_fiche_titre("Nuevo tipo de documento");
    ...
    print '<input type="hidden" name="action" value="add">'; ///Esto es el action del formulario que llama a la funcion add que hay mas arriba
    ...
    print "<tr>";
    print '<td valign="top" class="fieldrequired" width="15%">Nombre del documento</td>';
    print '<td class="valeur"><input size="30" type="text" id="denominacion" name="denominacion" value=""></td></tr>';
    ...
    print "<tr>".'<td valign="top">Validez del documento (En días)</td>';
    print '<td class="valeur"><input size="30" type="text" id="validezdias" name="validezdias" value=""></td></tr>';
  ...
  /*  require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
    ...
    print '<div class="center"><input class="button" value="Crear" type="submit"></div>';
...
else
{
    if ($id)
    {
        ...
        if ($action == 'delete')
        {
            print $form->formconfirm($_SERVER['PHP_SELF']."?id=".$object->id,"Eliminar un tipo de documento","¿Está seguro de querer eliminar este tipo de documento?","confirm_delete", '',0,1);
        }
        ...
        if ($action != 'edit')
        {
            print_fiche_titre("Documento");
            //dol_fiche_head($head, 'group', $title, 0, 'group'); ////core/class/doleditor.class.php';
            ...
            // Name
            print '<tr><td width="25%" valign="top">Denominación del documento</td>';
            print '<td width="75%" class="valeur">'.$object->denominacion;
            ...
            // validezdias
            print '<tr><td width="25%" valign="top">Validez del documento (En días)</td>';
            //print '<td class="valeur">'.dol_htmlentitiesbr($object->validezdias).'&nbsp;</td>';
            print '<td class="valeur">'.$object->validezdias.' &nbsp;días</td>';
            print "</tr>\n";
...
*/
          ...
          /*  print_fiche_titre($langs->trans("ListOfUsersInGroup"),'','');
            ...
            if ($caneditperms)
            {
                ...
                print '<input type="submit" class="button" value="dd'.$langs->trans("Add").'">';
                print '</td></tr>'."\n";
                print '</table></form>'."\n";
                print '<br>';
            }
*/
        ...
        if ($action == 'edit' && $caneditperms)
        {
            ...
            //dol_fiche_head($head, 'group', $title, 0, 'group');
             print_fiche_titre("Modificar Tipo de Documento");
            print '<table class="border" width="100%">';
            print '<tr><td width="25%" valign="top" class="fieldrequired">Denominación</td>';
            print '<td width="75%" class="valeur"><input size="15" type="text" name="denominacion" value="'.$object->denominacion.'">';
            print "</td></tr>\n";
           ...
           /* if (! empty($conf->multicompany->enabled) && is_object($mc))
            ...
            print '<td class="valeur">';
            /*require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
            $doleditor=new DolEditor('validezdias',$object->validezdias,'',240,'dolibarr_notes','',true,false,$conf->global->FCKEDITOR_ENABLE_SOCIETE,ROWS_8,10);
            $doleditor->Create();*/
             print '<input size="30" type="text" id="validezdias" name="validezdias" value="'.$object->validezdias.'">';
