...
function print_left_eldy_menu($db,$menu_array_before,$menu_array_after,&$tabMenu,&$menu,$noout=0,$forcemainmenu='',$forceleftmenu='')
{
	...
	if (empty($conf->global->MAIN_MENU_INVERT) && empty($noout) && ! empty($conf->global->MAIN_SHOW_LOGO) && empty($conf->global->MAIN_OPTIMIZEFORTEXTBROWSER))
	{
		...
		print "\n".'<!-- Show logo on menu -->'."\n";
		print '<img src="/htdocs/core/tpl/imagenes/logonoterp.png" width="180px">';
		print '<div class="blockvmenuimpair">'."\n";
		print '<div class="menu_titre" id="menu_titre_logo"></div>';
		print '<div class="menu_top" id="menu_top_logo" style="font-weight:bold;">Empresa</div>';
		...
		if ($mainmenu == 'home')
		{
			...
			if ($user->rights->user->user->lire)
			{
				if (empty($leftmenu) || $leftmenu == 'none' || $leftmenu=="users")
				{
					...
					///Nuevo item del menu de usuarios - Nuevo tipo de documento
					//Los números indican la posicion del elemento, si es encabezado o subitem
					$newmenu->add("/user/documentos/card.php?action=create", "Nuevo tipo de documento", 2, $user->rights->user->user->creer || $user->admin);
					//Listado de documentos
					$newmenu->add("/user/documentos/index.php", "Listado de documentos", 2, $user->rights->user->user->creer || $user->admin);
