...
/*
 * Mensaje de confirmacion de eliminacion del archivo
 */

if ($action == 'delete') 
{
	///Obtengo el id del documento que lo he pasado por url 
	$iddoc=GETPOST("iddoc");
	
	$langs->load("companies");	// Need for string DeleteFile+ConfirmDeleteFiles

	//ahora llamamos a if ($action == 'confirm_deletefile' && $confirm == 'yes') del documento /core/tpl/document_actions_pre_headers.tpl.php
	
	$ret = $form->form_confirm(
			$_SERVER["PHP_SELF"] . '?id=' . $object->id .'&iddoc='.$iddoc.'&urlfile=' . urlencode(GETPOST("urlfile")) . '&linkid=' . GETPOST('linkid', 'int') . (empty($param)?'':$param),
