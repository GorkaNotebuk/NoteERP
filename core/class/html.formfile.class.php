...
class FormFile
{
    ...
    function form_attach_new_file($url, $title='', $addcancel=0, $sectionid=0, $perm=1, $size=50, $object='', $options='', $useajax=1, $savingdocmask='', $linkfiles=1, $htmlname='formuserfile')
    {
        ...
        $idusuario=$object->id; //Guardo en la variable el id del usuario para pasarsela en la funcion de anadir()
		...
		if ((! empty($conf->global->MAIN_USE_JQUERY_FILEUPLOAD) && $useajax) || ($useajax==2))
        {
           ...
           // $out .= '<form name="'.$htmlname.'" id="'.$htmlname.'" action="'.$url.'" enctype="multipart/form-data" method="POST">';
            $out .= '<form name="'.$htmlname.'" id="'.$htmlname.'" action="'.$this->anadir($idusuario).'" enctype="multipart/form-data" method="POST">';
         ...
         //  $out .= '<input type="hidden" name="action" value="'.$this->anadir($idusuario).'">'; //Cambio el action para que añada la foto en la base de datos
            ...
            $out .= '<input class="flat" type="file" name="userfile" id="userfile" size="'.$maxlength.'"';
            $out .= (empty($conf->global->MAIN_UPLOAD_DOC) || empty($perm)?' disabled':'');
            $out .= '>';
             $out .= '&nbsp;'; $out .= '&nbsp;';


        ///Para indicar el tipo de documento que es
                 $out .= 'Tipo de documento: ';
                 $out .= '&nbsp;';
                 ...
                 $out .=  '<select name="tiposdedocumentos" id="tiposdedocumentos" >';
                 $conexion0=mysql_connect("hl230.dinaserver.com", "erpnoteb", "jjdkd847FF") or die ("No se puede conectar al servidor");
                            mysql_select_db("erpnote") or die ("No se puede seleccionar la base de datos");
                            $consulta= "SELECT denominacion,docID from sdl_tiposdedocumentos ";
                            $consult=mysql_query($consulta,$conexion0);
                            //$row=mysql_fetch_array($consult);//En esta variable está guardadas todas las variables de la consulta select
                            ...
                            if(!empty($my_error)) {echo "Ha habido un error al insertar los valores".$my_error;}
                             ...
                             $out .=  '<option value="'.$fila["docID"].'"> '.$fila["denominacion"].'</option>';
                                      
                                }

                    //cerramos el select
                    $out .='</select>';
                    mysql_close($conexion0);
...
////Funcion para añadir el documento y el tipo de documento en la base de datos
function anadir($id='')
{
    global $user, $conf, $langs, $hookmanager;
    
    $IdUsuario=$id; //id de usuario del que son los documentos
    $IdDoc=$_POST["tiposdedocumentos"]; //Id del tipo de documento que es
    
    $fechaactual=strtotime('now');  
    $time = date("Y/m/d",$fechaactual); //Fecha en la que se sube el archivo
    
    $nombredoc="";
    $nombredoc=$_FILES['userfile']['name']; //Nombre del archivo


      if($nombredoc != ""){ ///Para que se inserte el documento en la base de datos con el tipo de documento que es tabla: sdl_documentacion
        
                $this->db->begin();
                $sql = "INSERT INTO ".MAIN_DB_PREFIX."documentacion (Idtipodoc,IdUsuario,nombredoc,Fechadoc) VALUES ('$IdDoc','$IdUsuario','$nombredoc','$time')";
	...
	function list_of_documents($filearray,$object,$modulepart,$param='',$forcedownload=0,$relativepath='',$permtodelete=1,$useinecm=0,$textifempty='',$maxlength=0,$title='',$url='', $showrelpart=0)
	{
		...
		else
		{
            ...
            ///añadir aqui el campo de validez
            print_liste_field_titre("Tipo de documento",$url,"tipo","",$param,'align="center"',$sortfield,$sortorder);
          
            print_liste_field_titre("Días de validez del documento",$url,"dias","",$param,'align="center"',$sortfield,$sortorder);
			...
			foreach($filearray as $key => $file)      // filearray must be only files here
			{
						...
						&& ! preg_match('/\.meta$/i',$file['name']))
				{
                 ...
                 $nombre= $file['name'];
                       // $this->obtenerdatos($id,$file['name']);

                        /////base de datos - Sacamos los datos del tipo de documento y la validez de los días
                        $this->db->begin();                        
                        $sql =" SELECT tipo.denominacion,tipo.validezdias,doc.Fechadoc,doc.Id FROM sdl_tiposdedocumentos tipo  INNER JOIN  sdl_documentacion doc  on doc.Idtipodoc=tipo.docID WHERE nombredoc = '$nombre' AND IdUsuario = '$id' ";
                        
                         $result=$this->db->query($sql);
                            if ($result) {
                                $obj = $this->db->fetch_object($result);
                               /*print $tipodoc=$obj->Idtipodoc;
                               print $idusuario=$obj->IdUsuario;
                               print $nombredoc=$obj->nombredoc;*/
                                $nombredoc=$obj->denominacion;
                               $validez=$obj->validezdias;
                               $fecha= $obj->Fechadoc;
                               $id_documento=$obj->Id;
                             ...
                             $this->db->commit();
                        /////
                             ////Esto es para mirar cuantos días quedan para que caduque el documento////

                            $fechaactual=strtotime('now');
                            $time = date("d/m/Y",$fechaactual);


                            $segundos=strtotime($fecha) - $fechaactual;
                            $diferencia_dias=intval($segundos/60/60/24);
                            $diferencia_dias= abs($diferencia_dias);

                            $diasquequeda=$validez-$diferencia_dias; //Estos son los días que quedan para que caduque el documento
					...
					*/
                    ...
                    print '<td align="center">'.$nombredoc.'<br> '.$validez.' días de validez</td>';
                    if($diasquequeda<'4' && $diasquequeda>'1'){
                    
                    print '<td align="center">  <span style="color:#FF000B !important">¡Caduca en '.$diasquequeda.' días!</span> </td>';

                    }else if($diasquequeda=='1'){print '<td align="center">  <span style="color:#FF000B !important">¡Caduca en '.$diasquequeda.' día!</span> </td>';


                    }else if($diasquequeda<'1'){
                    print '<td align="center">  <span style="color:#770100 !important">El documento se ha caducado</span> </td>';

                    }else{
                    print '<td align="center"> Caduca en '.$diasquequeda.' días </td>';
					...
					if ($permtodelete)
					{
						...
						*/
						...
						print '<a href="'.(($useinecm && $useajax)?'#':$url.'?action=delete&urlfile='.urlencode($filepath).$param).'&iddoc='.$id_documento.'" class="deletefilelink" rel="'.$filepath.'">'.img_delete().'</a>';
					}
					else print '&nbsp;';
					print "</td>";
					print "</tr>\n";
...
$this->db->begin();                        
$sql =" DELETE FROM ".MAIN_DB_PREFIX."documentacion WHERE Id= $id";
$result=$this->db->query($sql);
$this->db->commit();
